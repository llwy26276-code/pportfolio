...
import { renderGame, renderBrand, renderFilm, mediaPath, videoUrl } from '../scripts/render.mjs';
...
test('multiple selling points and images render with safe content',()=>{
  const work={id:'test',title:'<script>alert(1)</script>',selling_points:[{title:'one',images:[{image:'/assets/uploads/a.png'},{image:'/portfolio/assets/uploads/b.png'}]},{title:'two',images:[{image:'assets/uploads/c.png'}]}]};
...
test('URLs reject scripts and filesystem traversal',()=>{
  for(const path of ['../secret.png','/assets/uploads/../../secret.png','https://evil.test/a.png','assets/uploads/a.svg']) assert.throws(()=>mediaPath(path));
  assert.equal(mediaPath('/portfolio/assets/uploads/a.png'),'assets/uploads/a.png');
